...
('10000000-0000-0000-0000-000000000001', 'markdown', '{"text": "# バブルソート\n\nバブルソート（Bubble Sort）は、隣り合う要素を比較して交換を繰り返すことでデータを整列するアルゴリズムです。\n\n最もシンプルなソートアルゴリズムの一つであり、アルゴリズム学習の出発点として最適です。"}', 0),
('10000000-0000-0000-0000-000000000001', 'callout', '{"type": "info", "text": "計算量: 最悪 O(n²)、最良 O(n)（すでにソート済みの場合）、空間計算量: O(1)"}', 1),
('10000000-0000-0000-0000-000000000001', 'markdown', '{"text": "## アルゴリズムの仕組み\n\n1. 配列の先頭から隣り合う2つの要素を比較する\n2. 左の要素が右より大きければ交換する\n3. 配列の末尾まで繰り返す（1パス）\n4. 交換が発生しなくなるまでパスを繰り返す\n\n各パスで最大の要素が「泡（バブル）」のように末尾に浮かんでいくことから、この名前がつきました。"}', 2),
('10000000-0000-0000-0000-000000000001', 'code', '{"language": "python", "code": "def bubble_sort(arr):\n    n = len(arr)\n    for i in range(n):\n        swapped = False\n        for j in range(0, n - i - 1):\n            if arr[j] > arr[j + 1]:\n                arr[j], arr[j + 1] = arr[j + 1], arr[j]\n                swapped = True\n        if not swapped:\n            break\n    return arr\n\n# 例\nprint(bubble_sort([64, 34, 25, 12, 22, 11, 90]))\n# => [11, 12, 22, 25, 34, 64, 90]", "runnable": false}', 3),
('10000000-0000-0000-0000-000000000001', 'code', '{"language": "go", "code": "func bubbleSort(arr []int) []int {\n\tn := len(arr)\n\tfor i := 0; i < n; i++ {\n\t\tswapped := false\n\t\tfor j := 0; j < n-i-1; j++ {\n\t\t\tif arr[j] > arr[j+1] {\n\t\t\t\tarr[j], arr[j+1] = arr[j+1], arr[j]\n\t\t\t\tswapped = true\n\t\t\t}\n\t\t}\n\t\tif !swapped {\n\t\t\tbreak\n\t\t}\n\t}\n\treturn arr\n}", "runnable": false}', 4),
('10000000-0000-0000-0000-000000000001', 'callout', '{"type": "tip", "text": "最適化ポイント: swapped フラグにより、すでにソート済みの配列に対しては O(n) で完了します。"}', 5),
...
INSERT INTO lesson_blocks (lesson_id, block_type, content, sort_order) VALUES
('10000000-0000-0000-0000-000000000002', 'markdown', '{"text": "# マージソート\n\nマージソート（Merge Sort）は**分割統治法（Divide and Conquer）**に基づくソートアルゴリズムです。\n\n配列を半分に分割し、それぞれを再帰的にソートしてからマージ（統合）します。"}', 0),
('10000000-0000-0000-0000-000000000002', 'callout', '{"type": "info", "text": "計算量: すべてのケースで O(n log n)、空間計算量: O(n)"}', 1),
('10000000-0000-0000-0000-000000000002', 'markdown', '{"text": "## アルゴリズムの手順\n\n1. **分割（Divide）**: 配列を中央で2つに分割\n2. **統治（Conquer）**: 各半分を再帰的にマージソート\n3. **統合（Combine）**: ソート済みの2つの配列をマージ\n\n### マージの仕組み\n\n2つのソート済み配列から、先頭の小さい方を順に取り出して新しい配列に入れます。"}', 2),
('10000000-0000-0000-0000-000000000002', 'code', '{"language": "python", "code": "def merge_sort(arr):\n    if len(arr) <= 1:\n        return arr\n    \n    mid = len(arr) // 2\n    left = merge_sort(arr[:mid])\n    right = merge_sort(arr[mid:])\n    \n    return merge(left, right)\n\ndef merge(left, right):\n    result = []\n    i = j = 0\n    \n    while i < len(left) and j < len(right):\n        if left[i] <= right[j]:\n            result.append(left[i])\n            i += 1\n        else:\n            result.append(right[j])\n            j += 1\n    \n    result.extend(left[i:])\n    result.extend(right[j:])\n    return result\n\nprint(merge_sort([38, 27, 43, 3, 9, 82, 10]))\n# => [3, 9, 10, 27, 38, 43, 82]", "runnable": false}', 3),
('10000000-0000-0000-0000-000000000002', 'callout', '{"type": "warning", "text": "マージソートは追加のメモリ O(n) を必要とします。メモリ制約が厳しい場合はインプレースのソート（クイックソートなど）を検討しましょう。"}', 4),
('10000000-0000-0000-0000-000000000002', 'markdown', '{"text": "## バブルソートとの比較\n\n| | バブルソート | マージソート |\n|---|---|---|\n| 最悪計算量 | O(n²) | O(n log n) |\n| 安定性 | 安定 | 安定 |\n| 空間計算量 | O(1) | O(n) |\n| 実用性 | 教育用 | 実用的 |\n\nマージソートは大規模データで圧倒的に速く、安定ソートであるため実用的です。"}', 5);
...
INSERT INTO lesson_blocks (lesson_id, block_type, content, sort_order) VALUES
('10000000-0000-0000-0000-000000000003', 'markdown', '{"text": "# クイックソート\n\nクイックソート（Quick Sort）は、ピボット要素を基準にデータを分割する分割統治法アルゴリズムです。\n\n平均的に最も高速なソートアルゴリズムの一つで、多くのプログラミング言語の標準ライブラリで採用されています。"}', 0),
('10000000-0000-0000-0000-000000000003', 'callout', '{"type": "info", "text": "計算量: 平均 O(n log n)、最悪 O(n²)、空間計算量: O(log n)（再帰スタック）"}', 1),
('10000000-0000-0000-0000-000000000003', 'markdown', '{"text": "## アルゴリズムの手順\n\n1. **ピボット選択**: 配列から基準となる要素（ピボット）を選ぶ\n2. **分割（Partition）**: ピボットより小さい要素を左に、大きい要素を右に配置\n3. **再帰**: 左右それぞれに対して再帰的にクイックソートを適用"}', 2),
('10000000-0000-0000-0000-000000000003', 'code', '{"language": "python", "code": "def quick_sort(arr):\n    if len(arr) <= 1:\n        return arr\n    \n    pivot = arr[len(arr) // 2]\n    left = [x for x in arr if x < pivot]\n    middle = [x for x in arr if x == pivot]\n    right = [x for x in arr if x > pivot]\n    \n    return quick_sort(left) + middle + quick_sort(right)\n\nprint(quick_sort([3, 6, 8, 10, 1, 2, 1]))\n# => [1, 1, 2, 3, 6, 8, 10]", "runnable": false}', 3),
('10000000-0000-0000-0000-000000000003', 'markdown', '{"text": "## ピボット選択の重要性\n\nピボットの選び方が性能に大きく影響します:\n\n- **先頭要素**: ソート済みデータで最悪ケース O(n²) に\n- **中央要素**: バランスの良い分割になりやすい\n- **ランダム**: 最悪ケースを確率的に回避\n- **三値の中央値**: 先頭・中央・末尾の中央値を使用（実用的）"}', 4),
('10000000-0000-0000-0000-000000000003', 'callout', '{"type": "tip", "text": "クイックソートはインプレースで実装可能で、キャッシュ効率が良いため、実測性能ではマージソートを上回ることが多いです。"}', 5);
...
INSERT INTO lesson_blocks (lesson_id, block_type, content, sort_order) VALUES
('10000000-0000-0000-0000-000000000004', 'markdown', '{"text": "# 二分探索\n\n二分探索（Binary Search）は、**ソート済み配列**から目的の要素を効率的に見つけるアルゴリズムです。\n\n探索範囲を毎回半分に絞ることで、線形探索 O(n) に比べて O(log n) の計算量を実現します。"}', 0),
('10000000-0000-0000-0000-000000000004', 'callout', '{"type": "info", "text": "計算量: O(log n)、前提条件: 配列がソート済みであること"}', 1),
('10000000-0000-0000-0000-000000000004', 'markdown', '{"text": "## アルゴリズムの手順\n\n1. 配列の中央要素を確認\n2. 目的値と一致すれば探索終了\n3. 目的値が中央値より小さければ左半分を探索\n4. 目的値が中央値より大きければ右半分を探索\n5. 範囲がなくなるまで繰り返す"}', 2),
('10000000-0000-0000-0000-000000000004', 'code', '{"language": "python", "code": "def binary_search(arr, target):\n    left, right = 0, len(arr) - 1\n    \n    while left <= right:\n        mid = (left + right) // 2\n        if arr[mid] == target:\n            return mid\n        elif arr[mid] < target:\n            left = mid + 1\n        else:\n            right = mid - 1\n    \n    return -1  # 見つからなかった\n\narr = [2, 3, 4, 10, 40]\nprint(binary_search(arr, 10))  # => 3", "runnable": false}', 3),
('10000000-0000-0000-0000-000000000004', 'markdown', '{"text": "## 二分探索の応用\n\n二分探索は単純な値の検索以外にも幅広く応用できます:\n\n- **lower_bound / upper_bound**: 条件を満たす最初/最後の位置を見つける\n- **答えの二分探索**: 最適化問題で「条件を満たす最小/最大値」を効率的に求める\n- **回転ソート配列の探索**: 部分的にソートされた配列にも適用可能"}', 4);
...
('10000000-0000-0000-0000-000000000006', 'code', '{"language": "python", "code": "def dfs(graph, start):\n    visited = set()\n    result = []\n    \n    def _dfs(node):\n        visited.add(node)\n        result.append(node)\n        for neighbor in graph[node]:\n            if neighbor not in visited:\n                _dfs(neighbor)\n    \n    _dfs(start)\n    return result\n\nprint(dfs(graph, \"A\"))  # => [\"A\", \"B\", \"D\", \"E\", \"F\", \"C\"]", "runnable": false}', 4),
('10000000-0000-0000-0000-000000000006', 'callout', '{"type": "info", "text": "BFS は最短経路問題に、DFS はトポロジカルソートや閉路検出に適しています。問題の性質に応じて使い分けましょう。"}', 5);
...
INSERT INTO lesson_blocks (lesson_id, block_type, content, sort_order) VALUES
('10000000-0000-0000-0000-000000000007', 'markdown', '{"text": "# 連結リストの基礎\n\n連結リスト（Linked List）は、各要素（ノード）がデータと次のノードへの参照（ポインタ）を持つデータ構造です。\n\n配列とは異なり、メモリ上に連続して配置される必要がなく、挿入・削除が O(1) で行えます。"}', 0),
('10000000-0000-0000-0000-000000000007', 'callout', '{"type": "info", "text": "配列 vs 連結リスト: 配列はインデックスアクセス O(1)、連結リストは先頭への挿入/削除 O(1)。用途に応じて使い分けます。"}', 1),
('10000000-0000-0000-0000-000000000007', 'code', '{"language": "python", "code": "class ListNode:\n    def __init__(self, val=0, next=None):\n        self.val = val\n        self.next = next\n\nclass LinkedList:\n    def __init__(self):\n        self.head = None\n    \n    def prepend(self, val):\n        \"\"\"先頭に追加: O(1)\"\"\"\n        self.head = ListNode(val, self.head)\n    \n    def append(self, val):\n        \"\"\"末尾に追加: O(n)\"\"\"\n        if not self.head:\n            self.head = ListNode(val)\n            return\n        curr = self.head\n        while curr.next:\n            curr = curr.next\n        curr.next = ListNode(val)\n    \n    def delete(self, val):\n        \"\"\"値を削除: O(n)\"\"\"\n        if self.head and self.head.val == val:\n            self.head = self.head.next\n            return\n        curr = self.head\n        while curr and curr.next:\n            if curr.next.val == val:\n                curr.next = curr.next.next\n                return\n            curr = curr.next\n    \n    def to_list(self):\n        result = []\n        curr = self.head\n        while curr:\n            result.append(curr.val)\n            curr = curr.next\n        return result", "runnable": false}', 2),
